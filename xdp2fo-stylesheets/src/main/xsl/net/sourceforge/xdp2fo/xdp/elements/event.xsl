...
<!--
 | $Id: event.xsl,v 1.3 2008/07/10 12:27:51 rlangbehn Exp $
 |
...
<xsl:stylesheet
	...
	<!-- event element                                                     -->
	<!-- ================================================================= -->
	<xsl:template match="xft:event">
		<xsl:call-template name="log:trace" />
		<xsl:apply-templates select="@*" />
		<xsl:apply-templates select="xft:extras" />
		<xsl:apply-templates select="xft:execute" />
		<xsl:apply-templates select="xft:script" />
		<xsl:apply-templates select="xft:signData" />
		<xsl:apply-templates select="xft:submit" />
	</xsl:template>

	<!-- ================================================================= -->
	<!-- event properties                                                  -->
	<!-- ================================================================= -->
	<xsl:template match="xft:event/@activity">
		<xsl:call-template name="log:trace" />
	</xsl:template>
	
	<xsl:template match="xft:event/@name">
		<xsl:call-template name="log:trace" />
	</xsl:template>
	
	<xsl:template match="xft:event/@ref">
		<xsl:call-template name="log:trace" />
	</xsl:template>
